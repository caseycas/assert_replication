INSERT INTO assert_july_2015_no_merge1.non_function_agg(
...
SELECT project, sha, language, file_name, is_test, method_name, sum(assertion_add), sum(assertion_del), sum(total_add), sum(total_del), is_bug, author, commit_date  
  FROM assert_july_2015_no_merge1.non_function GROUP BY project, sha, language, file_name, is_test, method_name,is_bug, author, commit_date;